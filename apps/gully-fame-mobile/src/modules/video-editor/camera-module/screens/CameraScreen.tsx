...
// Stable empty array constant — created once at module load, reused across all renders
...
const CameraScreen: React.FC<CameraScreenProps> = ({ onBack, onNext, initialClips = EMPTY_CLIPS }) => {
  ...
  // 🔥 SAFETY: If camera stuck on "Flipping" for > 8 seconds, force reset the state
  useEffect(() => {
    if (isSwitchingLens) {
      const timeout = setTimeout(() => {
        console.warn('⚠️ [CameraStuck] isSwitchingLens stuck for 8+ seconds. Forcing reset. This indicates camera initialization failure.');
        setIsSwitchingLens(false);
